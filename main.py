...
# -5 so border is equal size on both sides
...
YELLOW_PLAYER_HIT = pygame.USEREVENT + 1
RED_PLAYER_HIT = pygame.USEREVENT + 2
...
BULLET_FIRE_SOUND = pygame.mixer.Sound(os.path.join('Assets', 'short-explosion.wav'))
WINNER_SOUND = pygame.mixer.Sound(os.path.join('Assets', 'clapping-crowd.wav'))
...
def yellow_player_movement(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_w] and yellow.y - VELOCITY > 0: # UP key
        yellow.y -= VELOCITY
    if keys_pressed[pygame.K_s] and yellow.y + VELOCITY + yellow.width < HEIGHT: # DOWN key
        yellow.y += VELOCITY
...
def red_player_movement(keys_pressed, red):
    ...
    if keys_pressed[pygame.K_UP] and red.y - VELOCITY > 0: # UP key
        red.y -= VELOCITY
    if keys_pressed[pygame.K_DOWN] and red.y + VELOCITY + red.width < HEIGHT: # DOWN key
        red.y += VELOCITY
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_PLAYER_BULLETS:
                    bullet = pygame.Rect(
                        yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_PLAYER_BULLETS:
                    bullet = pygame.Rect(
